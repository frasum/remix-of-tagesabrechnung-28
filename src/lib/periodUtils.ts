...
/**
 * Returns the period range (26. prev month – 25. given month) for a given month/year.
 */
export function getPeriodRange(month: number, year: number) {
  // month is 1-12
  const start = new Date(year, month - 2, 26); // month-2 because JS is 0-based
...
/**
 * Generates Mo-So weeks for a period range.
 */
...
/**
 * Ensures periods exist for ±monthsRange months around today for the given restaurant.
 * Idempotent: skips periods that already exist (matched by start_date + restaurant_id).
 */
export async function ensurePeriodsExist(restaurantId: string, pastMonths = 1, futureMonths = 3) {
  ...
  // Build list of (month, year) tuples to ensure
  ...
  // Fetch existing periods for this restaurant
  ...
  for (const { month, year } of targets) {
    ...
    const { error: weekError } = await supabase.from("weeks").insert(weeks as any);
