...
CREATE OR REPLACE FUNCTION public.compute_carry_over(
...
BEGIN
  ...
  -- Iterate day by day over all dates (sessions + transfer-only) before p_before_date
  FOR rec IN (
  ...
  LOOP
    -- Calculate raw bargeld for this day
    IF rec.session_id IS NOT NULL THEN
      -- Get open invoices from waiter_shifts
    ...
    -- Calculate transfer effect for this date
    SELECT COALESCE(SUM(
    ...
    v_carry_over := CASE WHEN v_bargeld < 0 THEN v_bargeld ELSE 0 END;
  END LOOP;
